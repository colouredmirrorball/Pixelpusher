...
public class PixelPusher extends DeviceImpl
        implements Comparable<PixelPusher> {
    /**
    ...
    public void increaseExtraDelay(long i) {
        if (autothrottle) {
            extraDelayMsec += i;
            System.err.println(
                    "Group " + groupOrdinal + " card " + controllerOrdinal + " extra delay now " + extraDelayMsec);
        } else {
            System.err.println("Group " + groupOrdinal + " card " + controllerOrdinal +
                    " would increase delay, but autothrottle is disabled.");
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getPixelsPerStrip() != other.getPixelsPerStrip())
            return false;
        return getNumberOfStrips() == other.getNumberOfStrips();
    }
    ...
    public int compareTo(PixelPusher comp) {
        ...
        int group1 = comp.getGroupOrdinal();
        if (group0 != group1) {
            if (group0 < group1)
                return -1;
            return 1;
        ...
        int ord1 = comp.getControllerOrdinal();
        if (ord0 != ord1) {
            if (ord0 < ord1)
                return -1;
            return 1;
